...
def siraj_fun(aau_array,e_array,irad_array,wrad_array,Wrad_array,Mrad_array,probmasses_array,delta,n_q,n_p):
    ...
    pval_mat_siraj_2 = logLmax_siraj/logL_mat_siraj
...
# libration = 'gplusminusnone_2026feb12'
    libration = libration + '_2026feb12'
    ...
    df_vtvec.to_csv('b020_2026feb12_plutinos_vtvec_'+libration+'.csv',index=False)
    ...
    df_vmf.to_csv('b020_2026feb12_plutinos_vmf_'+libration+'.csv',index=False)
    ...
    df_fortran.to_csv('b020_2026feb12_plutinos_fortran_'+libration+'.csv',index=False)
#%%
for libration in ['gplus','gminus','gplusminusnone']:
    libration = libration + '_2026feb12'
# for libration in ['gplusminusnone']:
    print(libration)
    dfind = pd.read_csv('b004_p3q2_'+libration+'_index.csv')
    indices = dfind['index'].to_list()
    infile = 'b004_tnos_orbels_jd246e4.csv'
    ...
    ideg_siraj,Wdeg_siraj,q_siraj,p_siraj,s_siraj,angledegs_siraj,\
        ellipse_params_siraj,gamma_siraj,logLmax_siraj,logL_mat_siraj,sigma_mat_siraj,pval_mat_siraj,\
        q_mat_siraj,p_mat_siraj,pval_mat_siraj_2 = \
    ...
    outfile_siraj = 'b020_plutinos_'+libration+'_siraj_logLmat_delta'+str(delta)+'_nq'+str(n_q)+'_np'+str(n_p)+'.csv'
    np.savetxt(outfile_siraj,logL_mat_siraj,delimiter=',')
    outfile_siraj = 'b020_plutinos_'+libration+'_siraj_pvalmat_delta'+str(delta)+'_nq'+str(n_q)+'_np'+str(n_p)+'.csv'
    np.savetxt(outfile_siraj,pval_mat_siraj,delimiter=',')
    outfile_siraj = 'b020_plutinos_'+libration+'_siraj_qmat_delta'+str(delta)+'_nq'+str(n_q)+'_np'+str(n_p)+'.csv'
    np.savetxt(outfile_siraj,q_mat_siraj,delimiter=',')
    outfile_siraj = 'b020_plutinos_'+libration+'_siraj_pmat_delta'+str(delta)+'_nq'+str(n_q)+'_np'+str(n_p)+'.csv'
    np.savetxt(outfile_siraj,p_mat_siraj,delimiter=',')
    outfile_siraj = 'b020_plutinos_'+libration+'_siraj_sigmamat_delta'+str(delta)+'_nq'+str(n_q)+'_np'+str(n_p)+'.csv'
    ...
    df_siraj.to_csv('b020_plutinos_'+libration+'_siraj.csv',index=False)